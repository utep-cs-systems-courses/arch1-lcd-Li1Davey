...
main()
{
  ...
  /*
  for (short c = 60; c >= 0; c--){
    for(short j = 0; j <= c; j++){
      short R = 40 - c;
      drawPixel(centerC+j, centerR+R, COLOR_RED); //bottom right
      drawPixel(centerC+j, centerR-R, COLOR_YELLOW); //top right
      drawPixel(centerC-j, centerR+R, COLOR_GREEN); //bottom left
      drawPixel(centerC-j, centerR-R, COLOR_PINK); //top left
    }
  }
